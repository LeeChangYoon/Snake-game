...
void Eliminate(char *str, char ch); // Eliminates the specific char
...
void bomb(void)
{
	...
	while (1)
	{
		...
		for (j = 0; j < num_of_bombs; j++)
		{
			bomb_x[j] = (rand() % (MAP_X - 2)) + 1;
			bomb_y[j] = (rand() % (MAP_Y - 2)) + 1;
		}
		for (j = 0; j < num_of_bombs; j++)
		{
			for (i = 0; i < length; i++)
			{
				...
				if (bomb_x[j] == food_x && bomb_y[j] == food_y) // Check if the bomb coordinates with the snake
				{
					bomb_crush_on = 1;
					r++;
					break;
